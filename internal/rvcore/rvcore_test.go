...
func TestEncodeRType(t *testing.T) {
	result := EncodeRType(instrMap["add"].Opcode, X7, instrMap["add"].Funct3, X5, X0, instrMap["add"].Funct7)
	expected := 0b0000000_00000_00101_000_00111_0110011
	if result != uint32(expected) { // ADD x7, x5, x0
		t.Errorf("want %b but got %b", expected, result)
	}
}
...
func TestEncodeBType(t *testing.T) {
	result := EncodeBType(0b1100011, 0b1_1000_0000_0000, 0b000, 0b00010, 0b00110)
...
func TestEncodeJType(t *testing.T) {
	result := EncodeJType(0b1101111, 0b00111, 0b1001_0111_1111_1111_0000)
...
func TestLexeLineAndRefineTokens(t *testing.T) {
	...
	tokens2, err = tokens2.RefineTokens()
	if err != nil {
		t.Errorf("unexpected error: %v", err)
	}
	...
	tokens3, err = tokens3.RefineTokens()
	if err != nil {
		t.Errorf("unexpected error: %v", err)
	}
